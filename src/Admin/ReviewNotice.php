...
class ReviewNotice {
	...
	public function maybe_display_notice(): void {
		...
		if ( $screen && 'toplevel_page_flavor-link-checker' === $screen->id ) {
			return;
		}
		...
		$review_url = 'https://wordpress.org/support/plugin/flavor-link-checker/reviews/#new-post';
		?>
		<div id="flc-review-notice" class="notice notice-info is-dismissible" style="position: relative;">
			<p>
				<strong><?php \esc_html_e( 'How do you like Smart Link Checker?', 'flavor-link-checker' ); ?></strong><br>
				<?php \esc_html_e( 'We hope the plugin is helping you maintain a healthy site! If you have a moment, could you please leave us a 5-star rating on WordPress.org? It helps us a lot!', 'flavor-link-checker' ); ?>
			</p>
			<p>
				<a href="<?php echo \esc_url( $review_url ); ?>" class="button button-primary" target="_blank" rel="noopener noreferrer">
					<?php \esc_html_e( 'Leave a Review', 'flavor-link-checker' ); ?>
				</a>
				<button type="button" class="button button-link flc-dismiss-review" style="margin-left: 10px;">
					<?php \esc_html_e( 'Maybe later', 'flavor-link-checker' ); ?>
				</button>
			</p>
		</div>
		<?php
